...
async function getEcommerces(request, response, next) {
  try {
    const ecommerces = await usersService.getEcommerces();
    return response.status(200).json(ecommerces);
  } catch (error) {
    return next(error);
  }
}
...
/**
 * Handle create e-commerce request
 * @param {object} request - Express request object
 * @param {object} response - Express response object
 * @param {object} next - Express route middlewares
 * @returns {object} Response object or pass an error to the next route
...
async function createEcommerce(request, response, next) {
  try {
    ...
    const success = await usersService.createEcommerce(
      product,
      description,
      price,
      quantity
    ...
    if (!success) {
      throw errorResponder(
        errorTypes.UNPROCESSABLE_ENTITY,
        'Failed to create e-commerce'
      );
    }

    return response
      .status(200)
      .json({ message: 'Purchase already registered!' });
  } catch (error) {
    return next(error);
  }
}
...
async function updateEcommerce(request, response, next) {
  try {
    ...
    const success = await usersService.updateEcommerce(id, price, quantity);
    if (!success) {
      throw errorResponder(
        errorTypes.UNPROCESSABLE_ENTITY,
...
async function deleteEcommerce(request, response, next) {
  try {
    ...
    const success = await usersService.deleteEcommerce(id);
    if (!success) {
      throw errorResponder(
        errorTypes.UNPROCESSABLE_ENTITY,
